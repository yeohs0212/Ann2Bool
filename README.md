...
환경 충돌을 방지하기 위해 제공되는 `ann2bool_environment.yml` 파일을 사용하여 동일한 Conda 가상환경(`ann2bool`)을 간편하게 구축할 수 있습니다. (scvi 전처리부터 진행하려면 `scvi_env.yml` 파일을 사용하여 또 다른 가상환경을 사용하여 재현할 수 있습니다.)
